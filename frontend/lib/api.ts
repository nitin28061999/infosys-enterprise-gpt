/**
 * Central API client — matches the REAL backend routes in
 * backend/src (one router per feature folder), verified directly against
 * the repo rather than guessed. Every network call in the app goes through
 * this file.
 *
...
/** Backend wraps most responses as { success, message, data }. This unwraps it. */
...
/** For endpoints wrapped as { success, message, data } */
...
/** For endpoints that return the raw JSON body directly (no envelope) */
...
// ---------- Auth ----------
// POST /api/auth/signin expects OAuth2PasswordRequestForm — i.e. a
...
export const authApi = {
  ...
  // have no auth guard at all, which is a backend-side issue to fix before
  // any UI calls them).
...
// ---------- Query / Chat ----------
// POST /api/query/ returns the raw object directly (no {success,message,data}
// envelope). There is no endpoint to fetch prior chat history — the backend
// only exposes "ask a question", not "list my conversation".
...
// ADMIN / KNOWLEDGE_OWNER roles on the backend (employee_only will 403).
...
// this is the only analytics/metrics endpoint the backend exposes. There is
// no separate dashboard-stats endpoint.
...
export const feedbackApi = {
  ...
  getMine: (page = 1) =>
    requestPlain<{ data: FeedbackData[]; total: number; page: number; limit: number }>(
