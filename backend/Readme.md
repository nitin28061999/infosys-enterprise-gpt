...
The following points are visible in the uploaded source and should be
considered before calling the backend production-ready.

## 25.1 Department filtering is NOT performed directly in the initial vector search

The current query engine does:
...
``` python
can_access_document(doc.metadata, permission_context)
```

So department/permission filtering is currently **post-retrieval**, not
a Chroma metadata filter.

This still prevents unauthorized chunks from being passed into the LLM
if `can_access_document()` works correctly, but it may retrieve
unnecessary unauthorized chunks first.

------------------------------------------------------------------------

## 25.2 Admin permission-context key has a typo

The admin context currently contains:

``` python
"can_access-all_documemnt": True
```

while the authorization function expects:

``` python
permission_context["can_access_all_documents"]
```

These keys do not match.

This can cause an error for admin query authorization.

The key should consistently be:
...
## 25.3 Document list/get APIs do not apply document-level permissions

The RAG engine applies:
...
currently return database documents without calling:

``` text
can_access_document()
```

If these APIs are exposed to knowledge owners, document-level filtering
should be added.

------------------------------------------------------------------------

## 25.4 Document management APIs are inaccessible to employees

The document router has:

``` python
dependencies=[Depends(knowledge_owner_only)]
```

Therefore:

``` text
EMPLOYEE → cannot access /api/document/*
```

This is intentional for document management.

Employees interact with the knowledge base primarily through:
...
## 25.5 File upload supports PDF/DOCX/TXT, but the active indexer is PDF-oriented

Upload validation accepts:
...
However, the active `EnterprisePDFIndexer` uses:

``` python
PyMuPDFLoader
```

and creates a temporary file with:

``` text
.pdf
```

Therefore the current indexing implementation is primarily designed for
PDF content.

DOCX/TXT indexing should be implemented with appropriate loaders if
those formats are expected to be fully supported.

------------------------------------------------------------------------

## 25.6 Deleting a document does not clean up storage/vector data

Current deletion removes the PostgreSQL document row.

It does not explicitly remove:
...
A production implementation should use the document ID to delete all
associated vectors and the corresponding storage object.

------------------------------------------------------------------------

## 25.7 `source_system` is typed as boolean

The schema currently defines:

``` python
source_system: bool | None
```

but the document model defines it as:

``` python
String
```

These types should be aligned.

If the field is meant to store a source-system name, it should be a
string.
...
## 25.8 User deletion is a soft delete

Users are not physically removed.
...
This is useful for auditability and historical records.

However, the login implementation currently does not explicitly filter
out:
...
so this should be added to prevent deleted users from logging in.

------------------------------------------------------------------------

## 25.9 Admin/Knowledge Owner signup should be protected

These endpoints currently do not require JWT/RBAC:
...
For an enterprise production deployment, arbitrary clients should not be
able to create privileged accounts.

------------------------------------------------------------------------

## 25.10 Vector DB inspection should be restricted
...
This is appropriate for debugging but should normally be disabled or
restricted to administrators in production.

------------------------------------------------------------------------

# 26. Security Model Summary
...
RBAC
    ↓
ADMIN / KNOWLEDGE_OWNER / EMPLOYEE
...
Document Access Scope
    ↓
ALL / DEPARTMENT / OWNER

Layer 5
───────
Department / Owner comparison
    ↓
...
RAG grounding
    ↓
Only authorized retrieved context goes to Gemini

Layer 7
───────
Audit
    ↓
Record query, answer, retrieved documents and latency
```
...
``` text
src/documents/
services/uploadDocument_service.py
...
The backend implements an enterprise RAG workflow where:
...
Similarity Search
   ↓
RBAC + Document Permission Filtering
   ↓
Authorized Context
   ↓
...
> **A document should only become part of the LLM context after the
> user's role, confidentiality clearance, access scope, department
> and/or ownership have been evaluated.**
